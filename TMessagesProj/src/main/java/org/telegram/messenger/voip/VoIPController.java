...
public class VoIPController {
	...
	public void setEncryptionKey(byte[] key, boolean isOutgoing) {
		if (key == null || key.length != 256) {
			throw new IllegalArgumentException("key length must be exactly 256 bytes but is " + (key != null ? key.length : 0));
		}
		ensureNativeInstance();
	...
	/**
	 * Retrieves VoIP statistics.
	 * 
	 * @param stats Stats object to populate with current VoIP statistics.
	 *              Cannot be null.
	 * @throws NullPointerException if stats parameter is null
	 * @throws IllegalStateException if native instance is not initialized
	 */
	public void getStats(Stats stats) {
		ensureNativeInstance();
		if (stats == null) {
			throw new NullPointerException("Stats parameter cannot be null");
		}
	...
	private String getLogFilePath(String name) {
		...
		return new File(ApplicationLoader.applicationContext.getExternalFilesDir(null), String.format(Locale.US, "logs/%02d_%02d_%04d_%02d_%02d_%02d_%s.txt", c.get(Calendar.DATE), c.get(Calendar.MONTH), c.get(Calendar.YEAR), c.get(Calendar.HOUR_OF_DAY), c.get(Calendar.MINUTE), c.get(Calendar.SECOND), name)).getAbsolutePath();
	}
	...
	private native void nativeSetConfig(long inst, double recvTimeout, double initTimeout, int dataSavingOption, boolean enableAEC, boolean enableNS, boolean enableAGC, String logFilePath, String statsDumpPath, boolean debug);
